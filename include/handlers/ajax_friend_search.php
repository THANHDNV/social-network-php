    ...
    if ($query != '') {
        while($row = mysqli_fetch_array($usersReturned)) {
            ...
            if ($user->isFriend($row['username'])) {
                echo '<div class="resultDisplay"><a href="messages.php?u=' . $row["username"] . '" style="color: black"><div class="liveSearchProfilePic"><img src="' . $row["profile_pic"] . '"></div><div class="liveSearchText">' . $row["first_name"] . ' ' . $row["last_name"] . '<p style="margin: 0">' . $row["username"] . '</p>' . '<p id="grey">' . $mutual_friends . '</p></div></a></div>';
            }
        }
    }
